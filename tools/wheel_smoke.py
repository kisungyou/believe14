"""Smoke-test an installed believe14 wheel and its frozen public inventory."""
...
def main() -> None:
    ...
    _require(
        ...
        f"Smoke test imported the source checkout instead of the wheel: {module_path}",
    )
    _require(believe14.__version__ == "0.1.0", "Unexpected installed version.")

    installed = distribution("believe14")
    installed_files = {str(path) for path in installed.files or ()}
    _require("believe14/py.typed" in installed_files, "Wheel is missing py.typed.")
    _require(
        any(path.endswith("dist-info/licenses/LICENSE") for path in installed_files),
        "Wheel is missing its MIT license file.",
    )

    infos = list_estimators()
    _require(len(infos) == 30, "Wheel does not expose exactly 30 estimators.")
    _require(
        {info.family for info in infos} == {"linear", "nonlinear", "estimation"},
        "Wheel family inventory is incomplete.",
    )
    for info in infos:
        _require(
